...
//Initialization
void lattice_site::init_lattice_site()
{
        for(int i=0; i<4; i++)
                link[i].setIdentity();
        higgs.setZero();
}
void lattice_site::init_plattice_site()
{
        int i;
        FORALLDIR(i)
        link[i].setZero();
...
void lattice_site::init_lattice_site(std::mt19937_64& g)
{
        for(int i = 0; i < 4; i++)
                link[i] = uniformSU2Matrix(g);
        higgs = 0.005* normalHermitianMatrix(g);
}
void lattice_site::init_plattice_site(std::mt19937_64& g)
{
        int i;
        FORALLDIR(i)
